...
const initTextures = (n) => { 
    ...
    const u_Sampler0 = gl.getUniformLocation( gl.program,'u_Sampler0')//获取存储位置
    const u_Sampler1 = gl.getUniformLocation( gl.program,'u_Sampler1')//获取存储位置
    const image0 = new Image()//创建一个image
    const image1 = new Image()//创建一个image
    image0.src = wall
    image1.src = cubetexture
    image0.onload = () => { 
      loadTexture(n,texture0,u_Sampler0,image0,0)
     }
    image1.onload = () => { 
      loadTexture(n,texture1,u_Sampler1,image1,1)
     }
     return true
 }
 let g_texUnit0 = false,g_texUnit1 = false;
 const loadTexture = (n,texture,u_Sampler,image,texUnit) => { 
      console.log('执行了');
      ...
      gl.uniform1i(u_Sampler, texUnit) //纹理单元编号传给片元着色器中uniform变量u_Sampler
